...
def should_retain_snapshot(retention_plan:RetentionPlan) -> bool:
    ...
    if retention_plan.get_retention_type() == "STANDARD":
        return retention_plan_date >= datetime.now()

    else:
        next_snapshot_month = snapshot_date + timedelta(days=28)
        last_day_of_month = (next_snapshot_month.replace(day=28) + timedelta(days=4)) \
